...
const static int PMI_MASK = NHM_UNCORE_GLOBAL_CTRL_EN_PMI_CORE1;
...
void uncore_pmu_exit(void)
{
	...
	if (this_cpu == 6) {
		smp_call_function_single(XYZ, nhm_uncore_show_msrs, NULL, 1);
		smp_call_function_single(XYZ, nhm_uncore_clear_msrs, NULL, 1);
	}
	else {
		show();
		clear();
